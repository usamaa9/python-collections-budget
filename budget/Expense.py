...
class Expenses():
    ...
    def categorize_set_comprehension(self):
        essentials = ['Phone', 'Auto and Gas',
                      'Classes', 'Utilities', 'Mortgage']
        food_categories = ['Groceries', 'Eating Out']

        necessary_expenses = {x for x in self.list if x.category in essentials}

        food_expenses = {x for x in self.list if x.category in food_categories}
